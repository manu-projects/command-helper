...
* TODO GNU Make - Caracter # de comentario en Macros y Funciones Shell
...
# por tanto quedaría comentado el valor asignado a ésta macro
#+BEGIN_SRC makefile
...
# es necesario el slash invertido '\'  para escapar el caracter '#' porque GNU Make interpreta los # como comentarios,
# por tanto quedaría comentado parte de la función $(shell ...)
# y GNU Make lanzaría un error porque NO cerrarían los párentesis ) porque.. quedarón comentados...
#+BEGIN_SRC makefile
LINE_METADATA_OBSERVACIONES=$(shell grep -n '\#\# OBSERVACIONES:' $(DOC_COMMANDS_LINUX_DIRECTORY)/.template | cut --delimiter=':' --fields=1)
#+END_SRC
* TODO Shell - Comando grep para obtener número de linea de un archivo
#+BEGIN_SRC makefile
LINE_METADATA_OBSERVACIONES=$(shell grep -n '\#\# OBSERVACIONES:' $(DOC_COMMANDS_LINUX_DIRECTORY)/.template | cut --delimiter=':' --fields=1)
